...
import { siteTypes, templates } from "../../../data/orderPage";
import Title from "../components/UI/Title";
import Button from "../components/UI/Button";
import { AnimatePresence, motion } from "framer-motion";

// Для Headless UI Listbox
const typeOptions = [{ id: "all", name: "Все типы" }, ...siteTypes];
...
export default function OrderPage() {
  ...
  React.useEffect(() => {
    setCurrentPage(1);
  }, [templateTypeFilter, templateSearch]);
  ...
  const cardAnimation =
    "transition-all duration-300 ease-out transform hover:-translate-y-1 hover:shadow-2xl opacity-0 animate-fade-in";

  return (
    <main className="min-h-screen bg-[#0a0e1a] text-white font-sans pt-24">
      <Container>
        <Header />
        <Title className="text-center mb-10">Заказать сайт мечты</Title>
        <div className="px-10 mx-auto">
            ...
            {[1, 2, 3, 4].map((n) => (
              <div
                ...
                className={`w-8 h-2 rounded-full ${
                  step === n ? "bg-yellow-400" : "bg-gray-700"
                }`}
              />
            ))}
          </div>
            ...
            <div>
                ...
                {siteTypes.map((type) => (
                  <button
                    ...
                    style={{
                      boxShadow:
                        siteType === type.id
                          ? "0 4px 24px 0 rgba(255, 221, 51, 0.10)"
                          : undefined,
                    }}
              ...
              {siteType && (
                <div className="mb-6 p-4 bg-[#181f2e] rounded-xl border-l-4 border-yellow-400 text-neutral-200 animate-fade-in">
                  <b>{siteTypes.find((t) => t.id === siteType)?.name}:</b>{" "}
                  {siteTypes.find((t) => t.id === siteType)?.description}
                </div>
              )}
          ...
          {step === 2 && (
            <>
              <div className="max-w-2xl mx-auto px-4">
                <h2 className="text-2xl font-semibold mb-4">
                  2. Выберите макет сайта
                </h2>
                <div className="flex flex-col sm:flex-row gap-4 mb-10 px-4">
                          ...
                          <Listbox.Options className="absolute z-30 mt-2 max-h-60 w-full overflow-auto rounded-xl bg-[#181f2e] py-2 shadow-2xl ring-1 ring-black/20 focus:outline-none animate-fade-in border border-gray-700">
                            {typeOptions.map((option) => (
                              <Listbox.Option
                                ...
                                ${
                                  props.selected
                                    ? "font-bold bg-yellow-400/20 text-yellow-300"
                                    : ""
                                }`
                                }
                              >
              ...
              {(() => {
                ...
                if (templateTypeFilter !== "all") {
                  filtered = filtered.filter(
                    (tpl) =>
                      tpl.type === templateTypeFilter || tpl.id === "custom"
                  );
                }
                  ...
                  <>
                    <AnimatePresence mode="wait">
                      <motion.div
                        ...
                        className="grid grid-cols-1 sm:grid-cols-1 lg:grid-cols-3 md:grid-cols-2 gap-6 mb-6 max-w-6xl mx-auto min-h-[500px]"
                      >
                                ...
                                <button
                                  className={`px-4 py-2 rounded-lg font-semibold transition-all duration-200 ${
                                    selectedTemplate === tpl.id
                                      ...
                                      : "bg-gray-700 text-white hover:bg-yellow-400 hover:text-black hover:scale-105"
                                  }`}
                        ...
                        {paginatedTemplates.length === 0 && (
                          <div className="col-span-full text-center text-neutral-400 py-8">
                            Нет макетов по выбранным параметрам
                          </div>
                        )}
                      </motion.div>
                    </AnimatePresence>
                    ...
                    {filtered.length > pageSize && (
                      <div className="flex justify-center gap-2 mb-6">
                        <button
                          className="w-10 h-10 rounded-full bg-gray-700 text-white font-bold flex items-center justify-center transition hover:bg-yellow-400 hover:text-black disabled:opacity-40"
                          onClick={() =>
                        ...
                        {[...Array(totalPages)].map((_, i) => (
                          <button
                            key={i}
                            className={`w-10 h-10 rounded-full flex items-center justify-center font-bold transition border-2 mx-1 ${
                              currentPage === i + 1
          ...
          {step === 3 && (
            <div className="animate-fade-in px-6 max-w-2xl mx-auto">
              <h2 className="text-2xl font-semibold mb-4">
          ...
          {step === 4 && (
            <div className="animate-fade-in px-6 max-w-2xl mx-auto">
              ...
              <div className="bg-[#181f2e] rounded-xl p-6 mb-6">
                <div className="mb-2">
                  <b>Тип сайта:</b> {selectedSiteTypeObj?.name}
                </div>
                <div className="mb-2">
                  <b>Макет:</b> {selectedTemplateObj?.name}
                </div>
                <div className="mb-2">
